...
RovCameraCapture::RovCameraCapture(QWidget *parent) : QWidget(parent) {
    displayTimer = new QTimer(this);
    displayTimer->setInterval(16 + 2 / 3);
    displayTimer->start();
    connect(displayTimer, &QTimer::timeout, this,
            &RovCameraCapture::processCamera);
}

void RovCameraCapture::startCapture(int index = 2) {

    qWarning() << "Opening result: " << QString(cap.open(index));
    updateNeeded = true;
}

void RovCameraCapture::stopCapture() { cap.release(); }
