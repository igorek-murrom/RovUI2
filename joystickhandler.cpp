...
void JoystickHandler::updateSettings(){
    ...
    for(int i = 0; i < 6; i++)  //init axes from settings
        m_joystick.data()->axes_id[i] = m_settings->value(QString(SDL_JoystickName(m_sdlJoystick)) + QString("/joystickAxes/") + JoystickNames::axesNames[i]).toInt();
    for(int i = 0; i < 16; i++) //same for buttons
        m_joystick.data()->buttons_id[i] = m_settings->value(QString(SDL_JoystickName(m_sdlJoystick)) + QString("/joystickButtons/") + JoystickNames::buttonsNames[i]).toInt();
    for(int i = 0; i < 4; i++)  //same for hats
        m_joystick.data()->hats_id[i] = m_settings->value(QString(SDL_JoystickName(m_sdlJoystick)) + QString("/joystickHats/") + JoystickNames::hatsNames[i]).toInt();
}
...
void JoystickHandler::timerEvent(QTimerEvent *){
    ...
    if(SDL_NumJoysticks()>0){
        if(m_sdlJoystick != nullptr && joystickConnected == true){
            if(SDL_JoystickGetAttached(m_sdlJoystick)){ //joystick should exist and be opened
                for(size_t i = 0; i < helpers::size(m_joystick.data()->axes); i++){ // joystick input smoothing 3x
                    if(m_joystick.data()->axes_id[i]>=0){
                        float currentAxisVal = SDL_JoystickGetAxis(m_sdlJoystick, m_joystick.data()->axes_id[i]);
                        float lastAxisVal = m_joystick.data()->axes[i];
                        float secLastAxisVal = m_joystick.data()->axes_last[i];

                        m_joystick.data()->axes[i] = (map(currentAxisVal, -32768, 32767, -101, 101) + lastAxisVal + secLastAxisVal) / 3;
